...
async def save_result(user_id, score):
    get_stats = f"""
        ...
        WHERE user_id == $user_id;
    """
    stats = execute_select_query(pool, get_stats, user_id=user_id)
    await update_results_db(user_id, stats[0]["score"] + score)


async def update_results_db(user_id, score):
    update_results_db = f"""
        DECLARE $user_id AS Uint64;
        DECLARE $score AS Uint64;
        REPLACE INTO `quiz_results` (`user_id`, `score`) VALUES ($user_id, $score);
...
async def get_stats(user_id):
    ...
    if len(stats) == 0:
        return 0
    if stats[0]["score"] is None:
        return 0
    return stats[0]["score"]
